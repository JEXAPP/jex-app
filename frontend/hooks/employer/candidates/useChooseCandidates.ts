  ...
  type VacancyAPI = {
    ...
    shift_ids: number[];
    quantity?: number;         // cupo máximo de ofertas (vacante)
    quantity_offers?: number;  // ofertas realizadas (vacante)
  };
  ...
  // En este response asumimos datos por turno
  type ApplicationsByShiftAPI = {
    ...
    // Nuevo en este response (por turno)
    quantity?: number;         // cupo máximo de ofertas para este turno/vacante
    quantity_offers?: number;  // ofertas realizadas para este turno/vacante
  };
  ...
  export const useChooseCandidates = () => {
    ...
    // ofertas por turno (se cargan junto con horario+postulaciones)
    ...
    const fetchApplications = async (vacancyId: number | null, shiftId: number | null) => {
      ...
      try {
        ...
        // Ofertas por turno (preferimos valores del response de turno)
        ...
        // Fallback a nivel vacante si el shift no lo trae
    ...
    // Al cambiar vacante, setear primer turno (antes resetear para evitar combinaciones inválidas)
    useEffect(() => {
      if (!currentVacancy) {
        setSelectedShiftId(null);
    ...
    // Traer postulaciones + horario + ofertas por turno
    // Solo cuando el turno pertenece a la vacante seleccionada (evita 404)
    useEffect(() => {
    ...
    const handleSelectVacancy = (vacancyId: number) => {
      ...
      setSelectedShiftId(null);  // evita request con combinación vacante nueva + turno viejo
      setCandidates([]);
      setShiftInfo(null);
      setOffers(null);
    };
    ...
    return {
      ...
      shiftInfo,
      showShiftTags,
      // ofertas (se cargan junto a horario+postulaciones)
      offersMade,
      offersMax,
      isFull,
