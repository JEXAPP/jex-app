...
const clampDate = (d: Date) => {
  ...
  if (only < MIN_DATE) return MIN_DATE;
  if (only > MAX_DATE) return MAX_DATE;
  return only;
};
...
export function useSearchEmployees() {
  ...
  const onLocalidadChange = useCallback(
    async (text: string) => {
      ...
      const q = text.trim();
      if (q.length >= 3) {
        const res = await obtenerLocalidadesDeProvincia(selectedProvincia.descripcion, q);
        setLocalidadSug(res);
      } else {
        setLocalidadSug([]);
      }
  ...
  // Para DatePicker (detecta único vs rango)
  const applyDateSelection = useCallback((start: Date | null, end: Date | null) => {
    if (start && end && start.getTime() !== end.getTime()) {
      setStartDate(clampDate(start));
      setEndDate(clampDate(end));
      setIsRange(true);
    } else if (start) {
      setSingleDate(start);
    } else {
  ...
  // Normaliza si usuario pone fin < inicio en el mismo día
  ...
  // ===== Historial =====
  const [ratingMin, setRatingMin] = useState<number>(0);          // 0 = no enviar
  const [jobsMin, setJobsMin] = useState<number | null>(null);

  const decRating = useCallback(() => {
    setRatingMin(prev => Math.max(0, Math.round((prev - 0.5) * 2) / 2));
  }, []);
  const incRating = useCallback(() => {
    setRatingMin(prev => Math.min(5, Math.round((prev + 0.5) * 2) / 2));
  }, []);

  // ===== Acciones =====
  const validarParaBuscar = useCallback(() => {
    if (localidadInput && !selectedProvincia) {
      err('Primero seleccioná una provincia.');
      return false;
    }
    if (localidadInput && !selectedLocalidad) {
      err('Elegí una localidad de la lista de sugerencias.');
      return false;
    }
    if (startDate && (startDate < MIN_DATE || startDate > MAX_DATE)) {
      err('La fecha de inicio está fuera del rango permitido.');
      return false;
    }
    if (endDate && (endDate < MIN_DATE || endDate > MAX_DATE)) {
      err('La fecha de fin está fuera del rango permitido.');
      return false;
    }
    // Día único: si hay horas, inicio ≤ fin
    if (!isRange && startTime && endTime) {
      const [hi, mi] = startTime.split(':').map(Number);
      const [hf, mf] = endTime.split(':').map(Number);
      if (hi * 60 + mi > hf * 60 + mf) {
        err('En día único, la hora de inicio no puede ser mayor a la hora de fin.');
        return false;
      }
    }
    return true;
  ...
  const limpiarTodo = useCallback(() => {
    ...
    setRatingMin(0);
    setJobsMin(null);
    setActiveBubble('location');
  }, []);

  const onBuscar = () => {
  // Ejemplo: construir params desde tus variables finales
  const params = new URLSearchParams({
    ...
    ...(endDate && { end_date: ddmmyyyy(endDate) }),        // si es día único, igual al start_date
  ...
  const labels = {
    minDate: ddmmyyyy(MIN_DATE),
    maxDate: ddmmyyyy(MAX_DATE),
  };
  ...
  return {
    ...
    onProvinciaPick, onLocalidadChange, onLocalidadPick, onLocalidadClear,
    canPickLocalidad,
    ...
    showTimePickers, normalizeSingleDayTimes, labels,

    // Historial
    ratingMin, setRatingMin, jobsMin, setJobsMin,
    decRating, incRating,
