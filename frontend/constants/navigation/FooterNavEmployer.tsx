...
const ICON_SIZE = 24;
const FOOTER_HEIGHT = 65; // para usar en layout
  ...
  return (
    <View
      style={[
        s.container,
        {
          bottom: insets.bottom,   // levanta el footer por encima de la nav bar
          height: FOOTER_HEIGHT,   // altura fija del footer
          // sin paddingBottom con insets para no duplicar altura
        },
      ]}
    >
      <Pressable accessibilityRole="button" hitSlop={8} onPress={() => onPress(0)} disabled={isDisabled(0)} style={s.item}>
        {isActive(0) ? iconos.footer_home(true, ICON_SIZE, Colors.violet4) : iconos.footer_home(false, ICON_SIZE, Colors.violet4)}
      </Pressable>

      <Pressable accessibilityRole="button" hitSlop={8} onPress={() => onPress(1)} disabled={isDisabled(1)} style={s.item}>
        {isActive(1) ? iconos.footer_inbox(true, ICON_SIZE, Colors.violet4) : iconos.footer_inbox(false, ICON_SIZE, Colors.violet4)}
      </Pressable>

      <Pressable accessibilityRole="button" hitSlop={8} onPress={() => onPress(2)} disabled={isDisabled(2)} style={s.item}>
        {isActive(2) ? iconos.footer_briefcase(true, ICON_SIZE, Colors.violet4) : iconos.footer_briefcase(false, ICON_SIZE, Colors.violet4)}
      </Pressable>

      <Pressable accessibilityRole="button" hitSlop={8} onPress={() => onPress(3)} disabled={isDisabled(3)} style={s.item}>
        {isActive(3) ? iconos.footer_chat(true, ICON_SIZE, Colors.violet4) : iconos.footer_chat(false, ICON_SIZE, Colors.violet4)}
      </Pressable>

      <Pressable accessibilityRole="button" hitSlop={8} onPress={() => onPress(4)} disabled={isDisabled(4)} style={s.item}>
        {isActive(4) ? iconos.footer_person(true, ICON_SIZE, Colors.violet4) : iconos.footer_person(false, ICON_SIZE, Colors.violet4)}
      </Pressable>
    </View>
  );
};
