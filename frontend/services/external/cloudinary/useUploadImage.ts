...
export function useUploadImageServ() {
   ...
   * @param folderName Carpeta destino en Cloudinary:
   *  'events-images' | 'user-profiles-images' | 'certificates-docs' | 'work-experiences-docs'
   */
  const uploadImage = async (
    fileUri: string,
    folderName: 'events-images' | 'user-profiles-images' | 'certificates-docs' | 'work-experiences-docs'
  ): Promise<UploadResult> => {
    // 1) Pedir firma/credenciales al backend con la carpeta
    const respuesta = await requestBackend(
      `/media/cloudinary/sign/?folder=${folderName}`,
      null,
      'GET'
    );
    ...
    form.append('folder', folder || folderName);
