from django.urls import path
from applications.views.applications import ApplicationCreateView
from applications.views.offer import ApplicationDetailView, OfferCreateView, OfferConsultView
from applications.views.offers_confirms import OfferConfirmationCreateView

urlpatterns = [
    path('apply/', ApplicationCreateView.as_view(), name='apply'),
    path('apply/<int:pk>/detail/', ApplicationDetailView.as_view(), name='application-detail'),
    path('<int:application_id>/offer/', OfferCreateView.as_view(), name='offer-create'),
    path('offers/consult/', OfferConsultView.as_view(), name='offer-consult'),
    path('offers/<int:offer_id>/confirm/', OfferConfirmationCreateView.as_view(), name='offer-confirm')
]
